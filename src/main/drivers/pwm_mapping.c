...
static void pwmInitServos(timMotorServoHardware_t * timOutputs)
{
    ...
    for (int idx = 0; idx < servoCount; idx++) {
        uint16_t servoPwmRate = servoConfig()->servoPwmRate;
		if(servoPwmOverrides(idx)->servoPwmRate != 0)
		{
			servoPwmRate = servoPwmOverrides(idx)->servoPwmRate;
            LOG_V(PWM, "Servo %d PWM override to %d", idx, servoPwmRate);
		}

		const timerHardware_t *timHw = timOutputs->timServos[idx];

        if (!pwmServoConfig(timHw, idx, servoPwmRate, servoConfig()->servoCenterPulse, feature(FEATURE_PWM_OUTPUT_ENABLE))) {
            pwmInitError = PWM_INIT_ERROR_TIMER_INIT_FAILED;
